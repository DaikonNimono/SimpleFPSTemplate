...
void AFPSBombActor::BeginPlay()
{
	...
	TragetColor = FLinearColor::MakeRandomColor();
}
...
void AFPSBombActor::Explode()
{
	...
	for (FOverlapResult Result : OutOverlaps)
	{
		...
		if (Overlap && Overlap->IsSimulatingPhysics())
		{
			...
			if (MatInst) 
			{
				MatInst->SetVectorParameterValue("Color", TragetColor);
			}
		}
	}
	Destroy();
...
void AFPSBombActor::Tick(float DeltaTime)
{
	...
	if (MaterialInst)
	{
		...
		FLinearColor NewColor = FLinearColor::LerpUsingHSV(CurrentColor, TragetColor, Progress);
